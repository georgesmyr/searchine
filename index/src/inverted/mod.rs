...
pub trait Index {
    ...
    fn calc_idf(&self, term_id: u32) -> f64 {
        let n_docs_containing = self.n_docs_containing(term_id);
    ...
    fn calc_tf(&self, doc_id: u32, term_id: u32) -> f64 {
        let term_count = self.term_frequency(doc_id, term_id);
    ...
    fn calc_tf_idf(&self, doc_id: u32, term_id: u32) -> f64 {
        let tf = self.calc_tf(doc_id, term_id);
        let idf = self.calc_idf(term_id);
